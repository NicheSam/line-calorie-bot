...
- 會先判斷照片份量情境，例如一口、單顆、試吃、小碗、小紙盤、一般一人份或多人份。
- 對少量試吃、幾塊肉、局部特寫、單顆點心等情境，會避免直接套用完整主餐或完整一份的熱量。
- 支援快速刪除上一筆：`不記錄`、`這筆不算`。
- 支援復原上一動作：`復原`、`undo`。
...
- 針對少量 / 試吃 / 小份量照片，系統會跳過主餐型下限，避免把小碗雞腿肉、單顆水餃或少量炸物放大成完整一餐。
- Gemini 會輸出份量等級與用餐情境，讓後處理規則知道這是 `tiny`、`small`、`normal`、`large` 或 `unknown`。
- `修正學習` 可整理近期使用者修正，產生 FoodRules 建議草案。

### 估算限制

餐點照片估算仍然是粗估，不是精密營養秤。以下情境容易出錯：

- 照片缺乏比例尺，無法判斷實際重量。
- 醬料、油量、裹粉厚度、滷汁含糖量看不出來。
- 餐會試吃、分食、只拍局部、只剩幾口。
- 同一食物在不同店家差異很大，例如便當、滷味、早餐店、炸物。

這個 Bot 的策略是先保守估算，允許使用者用自然語言修正，再把修正累積成後續 FoodRules 與個人化建議。
